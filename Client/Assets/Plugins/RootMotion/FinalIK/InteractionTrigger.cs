...
namespace RootMotion.FinalIK
{
	...
	public class InteractionTrigger : MonoBehaviour
	{
		...
		public class CharacterPosition
		{
			...
			{
				get { return new Vector3(offset.x, 0f, offset.y); }
			}

			public Vector3 direction3D
			{
				get { return Quaternion.AngleAxis(angleOffset, Vector3.up) * Vector3.forward; }
			}
		...
		public class CameraPosition
		{
			...
			[Tooltip("Max angle between the direction and the direction towards the camera.")] [Range(0f, 180f)]
			public float maxAngle = 45f;
		...
		public class Range
		{
			...
			public bool IsInRange(Transform character, Transform raycastFrom, RaycastHit raycastHit, Transform trigger,
				out float maxError)
			{
				maxError = 0f;
				float error = 0f;
		...
		public int GetBestRangeIndex(Transform character, Transform raycastFrom, RaycastHit raycastHit)
		{
			...
			{
				if (ranges[i].IsInRange(character, raycastFrom, raycastHit, base.transform, out maxError) &&
				    maxError <= num)
				{
					num = maxError;
					result = i;
