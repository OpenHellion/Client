...
public abstract class MB3_MeshBakerRoot : MonoBehaviour
{
		...
		MB2_EditorMethodsInterface editorMethods, MB2_ValidationLevel validationLevel)
	{
		...
		if (mom is MB3_MeshBakerCommon)
		{
			...
			{
				Debug.LogWarning(
					"Material Bake Result on this component is not the same as the Material Bake Result on the MB3_TextureBaker.");
			}
		}
		...
		for (int i = 0; i < objectsToCombine.Count; i++)
		{
			...
			if (gameObject == null)
			{
				Debug.LogError("The list of objects to combine contains a null at position." + i +
				               " Select and use [shift] delete to remove");
				return false;
			}
			...
			{
				Debug.LogError(string.Concat("Object ", gameObject,
					" in the list of objects to be combined does not have a material"));
				return false;
			}
			...
			{
				Debug.LogError(string.Concat("Object ", gameObject,
					" in the list of objects to be combined does not have a mesh"));
				return false;
			}

			if (mesh != null && !Application.isEditor && Application.isPlaying &&
			    mom.textureBakeResults.doMultiMaterial && validationLevel >= MB2_ValidationLevel.robust)
			{
				...
				{
					Debug.LogWarning(string.Concat("Object ", objectsToCombine[i],
		...
		if (mom is MB3_MeshBaker)
		{
			...
			if (mom is MB3_MeshBaker &&
			    ((MB3_MeshBaker)mom).meshCombiner.renderType == MB_RenderType.skinnedMeshRenderer &&
			    !editorMethods.ValidateSkinnedMeshes(list))
			{
				return false;
			}
		}
