	...
	public class DockingPanelRadarTarget : MonoBehaviour
	{
		...
		private void Update()
		{
			if (!(TargetTrans == null) && !(ParentTrans == null))
			{
				...
				Vector3 vector = Vector3.ProjectOnPlane(TargetTrans.position - ParentTrans.position, ParentTrans.forward);
				...
				num3 = ((num4 < num5) ? (r1 * MathHelper.ProportionalValue(num4, 0f, num5, 0f, 1f)) : ((!(num4 >= num5) || !(num4 < 90f)) ? (r2 + (r3 - r2) * MathHelper.ProportionalValue(num4, 90f, 180f, 0f, 1f)) : (r1 + (r2 - r1) * MathHelper.ProportionalValue(num4, num5, 90f, 0f, 1f))));
