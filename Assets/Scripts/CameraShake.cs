...
public class CameraShake : MonoBehaviour
{
	...
	[ContextMenuItem("Shake", "ShkClick")] public float Duration = 2f;
	...
	{
		get { return targetMainMultiplier; }
		set { targetMainMultiplier = value; }
	}
	...
	private void Update()
	{
		...
		{
			test = false;
			StartCoroutine(Shake(shakeVariants[(int)testShakeType].shakeSpeed,
				shakeVariants[(int)testShakeType].shakeDuration, shakeVariants[(int)testShakeType].shakeMagnitude,
				shakeVariants[(int)testShakeType].xMultiplier, shakeVariants[(int)testShakeType].yMultiplier,
				shakeVariants[(int)testShakeType].rotXMultiplier, shakeVariants[(int)testShakeType].rotYMultiplier,
				shakeVariants[(int)testShakeType].rotZMultiplier, shakeVariants[(int)testShakeType].curve,
				indefiniteShake));
		}
	...
	public void ShakeCamera(ShakeType shakeType, bool infiniteShake)
	{
		if (!indefiniteShake)
		{
			...
			needToStop = false;
			StartCoroutine(Shake(shakeVariants[(int)shakeType].shakeSpeed, shakeVariants[(int)shakeType].shakeDuration,
				shakeVariants[(int)shakeType].shakeMagnitude, shakeVariants[(int)shakeType].xMultiplier,
				shakeVariants[(int)shakeType].yMultiplier, shakeVariants[(int)shakeType].rotXMultiplier,
				shakeVariants[(int)shakeType].rotYMultiplier, shakeVariants[(int)shakeType].rotZMultiplier,
				shakeVariants[(int)shakeType].curve, infiniteShake));
		}
	}
	...
	private IEnumerator Shake(float speed, float duration, float magnitude, float xMultiplier, float yMultiplier,
		float rotXMultiplier, float rotYMultiplier, float rotZMultiplier, AnimationCurve damper = null,
		bool infSHake = false)
	{
		float elapsed = 0f;
		...
		while (indefiniteShake)
		{
			...
			float x = (Mathf.PerlinNoise(Time.time * speed + 5f, 0f) * damperedMag - damperedMag / 2f) * xMultiplier *
			          currentMainMultiplier;
			float y = (Mathf.PerlinNoise(0f, Time.time * speed + 2.54f) * damperedMag - damperedMag / 2f) *
			          yMultiplier * currentMainMultiplier;
			float rotX = (Mathf.PerlinNoise(Time.time * speed + 18f, 0f) * damperedMag - damperedMag / 2f) *
			             rotXMultiplier * currentMainMultiplier;
			float rotY = (Mathf.PerlinNoise(0f, Time.time * speed + 100f) * damperedMag - damperedMag / 2f) *
	...
	public void CamShake(float duration, float amplitude, float rotationMultiplier, float frequency,
		bool useSparks = false)
	{
		StartCoroutine(CamShakeCoroutine(duration, amplitude, rotationMultiplier, frequency, useSparks));
	}

	private IEnumerator CamShakeCoroutine(float duration, float amplitude, float rotationMultiplier, float frequency,
		bool useSparks = false)
	{
		float time = 0f;
		if (useSparks)
