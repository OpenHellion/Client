...
namespace ZeroGravity.LevelDesign
{
	public class SceneDoor : MonoBehaviour, ISceneObject, ISoundOccludable
	{
		...
		private Quaternion defaultDoorUIRotation;
		...
		private void Start()
		{
			...
			SceneTriggerAnimation component = GetComponent<SceneTriggerAnimation>();
			if (component != null)
			{
		...
		public void SetDoorDetails(DoorDetails details, bool isInstant = false)
		{
			...
			pressureEquilizationTime = ((!(details.PressureEquilizationTime < 0.5f) || !(details.PressureEquilizationTime > 0f)) ? details.PressureEquilizationTime : 0.5f);
		...
		private void AffectInsideCorpses(Vector3 fLine, float fIntensity)
		{
			...
			foreach (Corpse corpse in componentsInChildren)
			{
				...
				if (vector.sqrMagnitude < 25f)
				{
					...
					{
						vector2 = vector3.normalized * ((airFlowDirection != 1) ? 1 : (-1)) * fIntensity;
					}
					else if (corpse.CurrentRoomTrigger == Room2)
					{
						vector2 = vector3.normalized * ((airFlowDirection != 2) ? 1 : (-1)) * fIntensity;
					}
					...
					{
						vector2 += -vector4 * fIntensity * 0.1f;
					}
		...
		private void AffectMyPlayer(Vector3 fLine, float fIntensity)
		{
			...
			if (airSpeed > 100f && !MyPlayer.Instance.FpsController.IsZeroG && !MyPlayer.Instance.FpsController.HasTumbled && (!InputManager.GetButton(InputManager.AxisNames.LeftShift) || !MyPlayer.Instance.FpsController.IsGrounded))
			{
				MyPlayer.Instance.FpsController.Tumble();
			}
			float num = ((!(MyPlayer.Instance.rigidBody.mass > 0f)) ? 10f : MyPlayer.Instance.rigidBody.mass);
			if (InputManager.GetButton(InputManager.AxisNames.LeftShift) && MyPlayer.Instance.FpsController.CanGrabWall)
			{
				num *= 100f;
			}
