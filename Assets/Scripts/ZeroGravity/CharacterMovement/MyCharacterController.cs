...
namespace ZeroGravity.CharacterMovement
{
	...
	public class MyCharacterController : MonoBehaviour
	{
		...
		{
			if (InputController.GetButton(InputController.AxisNames.LeftShift) && !IsCrouch && animatorHelper.CanRun)
			{
			...
			else if (InputController.GetButton(InputController.AxisNames.LeftCtrl))
			{
			...
			movementAxis.Forward = InputController.GetAxisRaw(InputController.AxisNames.Forward);
			movementAxis.Right = InputController.GetAxisRaw(InputController.AxisNames.Right);
		...
		private void update1GInput()
		{
			...
			if (isMovementEnabled && crouchLerpHelper.IsEpsilonEqual(1f, 0.001f) && isGrounded && InputController.GetButtonDown(InputController.AxisNames.Space))
			{
				doJump = true;
			}
		}
		...
		private void update1GMovement()
		{
			...
			if (isGrounded)
			{
				...
				if (InputController.GetButton(InputController.AxisNames.LeftCtrl))
				{
					lastMovementState = MovementState.Crouch;
				}
				CalculateCrouch();
				...
				else
				{
					...
					float? velocityRight = currRightAnimationVal;
					animatorHelper.SetParameter(null, null, null, null, null, null, null, null, null, null, velocityRight, ladderDirection);
					if (slopeJumpTimer > 1f && isGrounded && InputController.GetButtonDown(InputController.AxisNames.Space))
					{
			...
			{
				float axisRaw2 = InputController.GetAxisRaw(InputController.AxisNames.Forward);
				float axisRaw3 = InputController.GetAxisRaw(InputController.AxisNames.Right);
		...
		{
			// Get normal input.
			movementAxis.Forward = InputController.GetAxis(InputController.AxisNames.Forward);
			movementAxis.Right = InputController.GetAxis(InputController.AxisNames.Right);
			movementAxis.LeanRight = InputController.GetAxis(InputController.AxisNames.Lean);

			// Get vertical input.
			if (InputController.GetButton(InputController.AxisNames.Space))
			{
		...
		private void update0GMovement()
		{
			...
			else
			{
				...
				bool hasToStabilise = InputController.GetButton(InputController.AxisNames.LeftShift) || (myPlayer.IsUsingItemInHands && myPlayer.Inventory.CheckIfItemInHandsIsType<HandDrill>());
