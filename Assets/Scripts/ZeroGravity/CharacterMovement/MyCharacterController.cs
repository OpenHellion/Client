...
namespace ZeroGravity.CharacterMovement
{
	...
	public class MyCharacterController : MonoBehaviour
	{
		...
		{
			if (InputManager.GetButton(InputManager.AxisNames.LeftShift) && !IsCrouch && animatorHelper.CanRun)
			{
			...
			else if (InputManager.GetButton(InputManager.AxisNames.LeftCtrl))
			{
			...
			movementAxis.Forward = InputManager.GetAxisRaw(InputManager.AxisNames.Forward);
			movementAxis.Right = InputManager.GetAxisRaw(InputManager.AxisNames.Right);
		...
		private void update1GInput()
		{
			...
			if (isMovementEnabled && crouchLerpHelper.IsEpsilonEqual(1f, 0.001f) && isGrounded && InputManager.GetButtonDown(InputManager.AxisNames.Space))
			{
				doJump = true;
			}
		}
		...
		private void update1GMovement()
		{
			...
			{
				float axisRaw2 = InputManager.GetAxisRaw(InputManager.AxisNames.Forward);
				float axisRaw3 = InputManager.GetAxisRaw(InputManager.AxisNames.Right);
