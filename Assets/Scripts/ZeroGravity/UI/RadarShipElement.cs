...
namespace ZeroGravity.UI
{
	public class RadarShipElement : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			{
				Vector3 vector = Quaternion.LookRotation(PilotP.ParentShip.Forward, PilotP.ParentShip.Up).Inverse() *
				                 (AB.Position - PilotP.ParentShip.Position).ToVector3();
				...
				num = ((num2 < num3)
					? (r1 * MathHelper.ProportionalValue(num2, 0f, num3, 0f, 1f))
					: ((!(num2 >= num3) || !(num2 < 90f))
						? (r2 + (r3 - r2) * MathHelper.ProportionalValue(num2, 90f, 180f, 0f, 1f))
						: (r1 + (r2 - r1) * MathHelper.ProportionalValue(num2, num3, 90f, 0f, 1f))));
