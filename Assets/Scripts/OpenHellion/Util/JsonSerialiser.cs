...
namespace OpenHellion.Util
{
	public static class JsonSerialiser
	{
		...
		private static readonly AttachPointDataJsonConverter s_attachPointConverter = new AttachPointDataJsonConverter();

		public static T LoadPersistent<T>(string filename)
		{
			return JsonConvert.DeserializeObject<T>(File.ReadAllText(Path.Combine(Application.persistentDataPath, filename)), new JsonConverter[2] { s_auxConverter, s_attachPointConverter });
		}

		public static T LoadResource<T>(string resourcePath)
		{
			return JsonConvert.DeserializeObject<T>(Resources.Load(resourcePath).ToString(), new JsonConverter[2] { s_auxConverter, s_attachPointConverter });
		}

		public static T LoadDataPath<T>(string filePath)
		{
			return JsonConvert.DeserializeObject<T>(File.ReadAllText(Path.Combine(Application.dataPath, filePath)), new JsonConverter[2] { s_auxConverter, s_attachPointConverter });
		}

		public static T Load<T>(string filePath)
		{
			return JsonConvert.DeserializeObject<T>(File.ReadAllText(filePath), new JsonConverter[2] { s_auxConverter, s_attachPointConverter });
		}
		...
		{
			return JsonConvert.DeserializeObject<T>(jsonString, new JsonConverter[2] { s_auxConverter, s_attachPointConverter });
		}
