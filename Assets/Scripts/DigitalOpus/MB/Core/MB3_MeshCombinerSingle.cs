...
namespace DigitalOpus.MB.Core
{
	...
	public class MB3_MeshCombinerSingle : MB3_MeshCombiner
	{
		...
		public override MB2_TextureBakeResults textureBakeResults
		{
			set
			{
				if (objectsInCombinedMesh.Count > 0 && _textureBakeResults != value && _textureBakeResults != null && LOG_LEVEL >= MB2_LogLevel.warn)
				{
		...
		{
			if (objectsInCombinedMesh.Count > 0 && ((!_doNorm && doNorm) || (!_doTan && doTan) || (!_doCol && doCol) || (!_doUV && doUV) || (!_doUV3 && doUV3) || (!_doUV4 && doUV4)))
			{
		...
		private bool _addToCombined(GameObject[] goToAdd, int[] goToDelete, bool disableRendererInSource)
		{
			...
			if (LOG_LEVEL >= MB2_LogLevel.debug)
			{
				MB2_Log.LogDebug("==== Calling _addToCombined objs adding:" + _003C_addToCombined_003Ec__AnonStorey._goToAdd.Length + " objs deleting:" + array.Length + " fixOutOfBounds:" + textureBakeResults.fixOutOfBoundsUVs + " doMultiMaterial:" + textureBakeResults.doMultiMaterial + " disableRenderersInSource:" + disableRendererInSource, LOG_LEVEL);
			}
			...
			for (int num8 = 0; num8 < _003C_addToCombined_003Ec__AnonStorey._goToAdd.Length; num8++)
			{
				if (_003C_addToCombined_003Ec__AnonStorey._goToAdd[num8] != null && disableRendererInSource)
				{
					...
					{
						Debug.Log("Disabling renderer on " + _003C_addToCombined_003Ec__AnonStorey._goToAdd[num8].name + " id=" + _003C_addToCombined_003Ec__AnonStorey._goToAdd[num8].GetInstanceID());
					}
				}
			}
			...
			for (int num18 = 0; num18 < mbDynamicObjectsInCombinedMesh.Count; num18++)
			{
				...
				if (!mB_DynamicGameObject2._beingDeleted)
				{
					...
					for (int num19 = 0; num19 < num; num19++)
					{
						...
						if (LOG_LEVEL >= MB2_LogLevel.debug)
						{
							MB2_Log.LogDebug("    Adjusting submesh triangles submesh:" + num19 + " startIdx:" + num20 + " num:" + num21, LOG_LEVEL);
						}
