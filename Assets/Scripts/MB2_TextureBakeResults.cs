...
public class MB2_TextureBakeResults : ScriptableObject
{
	public class Material2AtlasRectangleMapper
	{
		...
		public Material2AtlasRectangleMapper(MB2_TextureBakeResults res)
		{
			...
			if (matsAndSrcUVRect == null || matsAndSrcUVRect.Length == 0)
			{
				...
				{
					matsAndSrcUVRect[i] = new MB_MaterialAndUVRect(res.materials[i], res.prefabUVRects[i],
						new Rect(0f, 0f, 1f, 1f), new Rect(0f, 0f, 1f, 1f), new Rect(0f, 0f, 1f, 1f), string.Empty);
				}
		...
		public bool TryMapMaterialToUVRect(Material mat, Mesh m, int submeshIdx,
			MB3_MeshCombinerSingle.MeshChannelsCache meshChannelCache,
			Dictionary<int, MB_Utility.MeshAnalysisResult[]> meshAnalysisCache, out Rect rectInAtlas,
			out Rect encapsulatingRect, out Rect sourceMaterialTilingOut, ref string errorMsg, MB2_LogLevel logLevel)
		{
			if (tbr.materialsAndUVRects.Length == 0 && tbr.materials.Length > 0)
			{
				errorMsg =
					"The 'Material Bake Result' needs to be re-baked to be compatible with this version of Mesh Baker. Please re-bake using the MB3_TextureBaker.";
			...
			if (mat == null)
			{
				...
				errorMsg = string.Format(
					"Mesh {0} Had no material on submesh {1} cannot map to a material in the atlas", m.name,
					submeshIdx);
				return false;
			}
			...
			if (!tbr.fixOutOfBoundsUVs)
			{
				...
				{
					Debug.LogError(
						"There is a problem with this TextureBakeResults. FixOutOfBoundsUVs is false and a material appears more than once.");
				}
			...
			if (logLevel >= MB2_LogLevel.trace)
			{
				Debug.LogWarning(string.Format(
					"Trying to find a rectangle in atlas capable of holding tiled sampling rect for mesh {0} using material {1}",
					m, mat));
			}
			...
			for (int k = num; k < matsAndSrcUVRect.Length; k++)
			{
				...
				if (logLevel >= MB2_LogLevel.trace)
				{
					Debug.Log("uvR=" + r.ToString("f5") + " matR=" + r2.ToString("f5") + "Potential Rect " +
					          rect.ToString("f5") + " encapsulating=" + r3.ToString("f5"));
				}
				...
				{
					Debug.Log(
						"Potential Rect Cannonical " + rect.ToString("f5") + " encapsulating=" + r3.ToString("f5"));
				}
				...
				if (MB3_UVTransformUtility.RectContains(ref r3, ref rect))
				{
					if (logLevel >= MB2_LogLevel.trace)
					{
						Debug.Log(string.Concat(
							"Found rect in atlas capable of containing tiled sampling rect for mesh ", m, " at idx=",
							k));
					}
			...
			sourceMaterialTilingOut = default(Rect);
			errorMsg = string.Format(
				"Could not find a tiled rectangle in the atlas capable of containing the uv and material tiling on mesh {0}",
				m.name);
			return false;
		}
	}
	...
	public static MB2_TextureBakeResults CreateForMaterialsOnRenderer(Renderer r)
	{
		...
		for (int i = 0; i < r.sharedMaterials.Length; i++)
		{
			...
			{
				MB_MaterialAndUVRect item = new MB_MaterialAndUVRect(r.sharedMaterials[i], new Rect(0f, 0f, 1f, 1f),
					new Rect(0f, 0f, 1f, 1f), new Rect(0f, 0f, 1f, 1f), new Rect(0f, 0f, 1f, 1f), r.name);
	...
	public bool ContainsMaterial(Material m)
	{
		if (materialsAndUVRects.Length == 0)
		{
			...
			{
				materialsAndUVRects[i] = new MB_MaterialAndUVRect(materials[i], prefabUVRects[i],
					new Rect(0f, 0f, 1f, 1f), new Rect(0f, 0f, 1f, 1f), new Rect(0f, 0f, 1f, 1f), string.Empty);
			}
		}
